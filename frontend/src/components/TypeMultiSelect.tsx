...
export function TypeMultiSelect({ pokemons, setFilters,filters,favorite, addToast,cenfil}: any) {
    ...
    const applyFilters = ( ) => {
        return pokemons.filter((p:any) => filters.types.every((ft:any) => p.types.some((pt:any) => pt.type.name === ft))
         && filters.abilities.every((fa:any) => p.abilities.some((pa:any)=> pa.ability.name === fa))
         && p.stats[0].base_stat > hpRange[0] && p.stats[0].base_stat < hpRange[1] && p.name.includes(filters.search)&& (p.stats[1].base_stat > attackRange[0] && p.stats[1].base_stat < attackRange[1]) &&
            (p.stats[2].base_stat > defenseRange[0] && p.stats[2].base_stat < defenseRange[1]) && (p.stats[3].base_stat > speedRange[0] && p.stats[3].base_stat < speedRange[1])) } 
    return (
        <div id="filter-panel">
             ...
             <div id="filter_btns">
                <button className='filter_btn' onClick={() => {
                    const poks = favChange ? applyFilters().filter((p:any) => favorite.includes(p.id)) : applyFilters()
                    poks.length == 0 ? addToast("error","No pokemons for that filters") : addToast("success","You filtered pokemons")
                    setFilters("add", "filtered",  poks)
                    setFilters("add", "minHp", hpRange[0])
                    setFilters("add", "maxHp", hpRange[1])
                    setFilters("add","showPanel",false)
                    setFilters("add","minAttack",attackRange[0])
                    setFilters("add","maxAttack",attackRange[1])
                    setFilters("add","minDefense",defenseRange[0])
                    setFilters("add","maxDefense",defenseRange[1])
                    setFilters("add","minSpeed",speedRange[0])
                    setFilters("add","maxSpeed",speedRange[1]) 
